...
public:
    /** Constructor
    */
    MenuEditor(wxWindow* parent = nullptr, int id = wxID_ANY);

    /** Rellena el wxListCtrl con los datos de "obj", que debe ser de tipo menubar
    */
    void Populate(PObjectBase obj);

    /** Devuelve la jerarquía de ObjectBase correspondiente a la barra de
        menús que hay en el wxListCtrl.

        El objeto devuelto será, por tanto, de tipo wxMenuBar
    */
...
private:
    ...
    */
    ...
    /** Inserta en la posición "n" un elemento en el wxListCtrl.

        No añade sangría en el campo label.

        @param obj Pointer to the original object to be saved with SetItemData
               If the item is still an item when the dialog is closed,
    ...
    */
    ...
    /** Inserta debajo del elemento seleccionado (o en la última posición en
        caso de no haber selección) un nuevo elemento en el wxListCtrl.

        El campo label se sangra al mismo nivel que el elemento que le precede
    */
    void AddItem(const wxString& label, const wxString& shortcut,
                 const wxString& id, const wxString& name, const wxString& help,
                 const wxString& kind);

    /** Determina el índice en el que acaba el menú que empieza en la posición "n"
    */
    long GetEndIndex(long n);

    /** Devuelve los campos del wxListCtrl del elemento "n".

        El campo label lo devuelve con sangría
    ...
    */
    ...
    /** Inserta en la posición "n" del wxListCtrl, los hijos de "obj".
        ...
        El primer hijo de "obj" estará sangrado con el valor de "ident".
        En "n" se devuelve la siguiente posición libre del wxListCtrl
        tras hacer la inserción
    */
    void AddChild(long& n, int ident, PObjectBase obj);

    /** Determina si el elemento "n" del wxListCtrl tiene submenús
        o por el contrario es un menuitem
    */
    bool HasChildren(long n);

    /** Devuelve el ObjectBase correspondiente al menú que empieza en la posición
        "n" del wxListCtrl, así como todos los hijos de ese menú.

        En "n" devuelve la posición en el wxListCtrl del siguiente menú.
        isSubMenu debe ser @true para los menús de primer nivel
        (aquellos que aparecerán en la barra de menús)
    ...
    */
    ...
    /** Toma los valores introducidos por el usuario en los campos e inserta
        el elemento en el wxListCtrl.

        Tras la inserción, vacía todos los campos y pone el foco en el campo "label".
    */
    void AddNewItem();
};
