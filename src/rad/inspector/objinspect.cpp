...
void ObjectInspector::SaveSettings()
{
    ...
    config->Write(wxT("/mainframe/propertyeditor/DescBoxHeight"), m_pg->GetDescBoxHeight());
}
...
void ObjectInspector::OnPropertyGridChanged(wxPropertyGridEvent& event)
{
    ...
    if (it != m_propMap.end()) {
        ...
        case PT_WXPOINT: {
            ...
            ModifyProperty(
                prop, wxString::Format(wxT("%i,%i"), point.x, point.y));
            break;
        }
        case PT_WXSIZE: {
            wxSize size = wxSizeRefFromVariant(event.GetPropertyValue());
            ModifyProperty(
                prop, wxString::Format(wxT("%i,%i"), size.GetWidth(), size.GetHeight()));
            break;
        }
        case PT_WXFONT: {
                                                                  ...
                                                                  wxWindowID id)
{
    ...
    wxConfigBase* config = wxConfigBase::Get();
    config->Read(
        "/mainframe/propertyeditor/DescBoxHeight", &descBoxHeight, defaultDescBoxHeight);
