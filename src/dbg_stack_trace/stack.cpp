...
public:
        ...
        , symbol_table_(0)
    {
        ...
        // Load symbol table
        unsigned dummy = 0;
        if (bfd_read_minisymbols(abfd_, FALSE, (void**)&symbol_table_, &dummy) == 0 && bfd_read_minisymbols(abfd_, TRUE, (void**)&symbol_table_, &dummy) < 0) {
            free(symbol_table_);
            bfd_close(abfd_);
            throw dbg::stack_error("Failed to parse object data for the executable");
        }
...
public:
    ...
    explicit auto_cast_function_ptr(FuncPtr f)
        : fptr_(reinterpret_cast<void (*)(void)>(f))
    {
    }
